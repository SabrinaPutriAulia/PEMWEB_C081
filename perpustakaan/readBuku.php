            ...
            <table class="table table-striped table-sm" style="text-align: center">
              ...
              <tbody>
                <?php 
                 ...
                 <!-- Menyimpan (menulis) data buku ke dalam file buku.txt-->
                 <?php 
                    $query = "SELECT * FROM buku";
                    $result = mysqli_query(connection(), $query);
                    
                    while ($row = mysqli_fetch_assoc($result)) {
                        $data .= implode("\t", $row) . PHP_EOL; // Menggabungkan data dengan tab sebagai pemisah dan tambahkan baris baru (new line)
                    }
                    
                    file_put_contents('buku.txt', $data);
